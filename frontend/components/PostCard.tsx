  ...
  return (
    <article className="rounded-xl p-6 shadow-xl hover:shadow-2xl flex flex-col justify-between gap-7 transition-shadow duration-200 md:max-w-90">
      <div className="flex flex-col justify-center gap-4">
        {post.postImage && (
          <Image src={post.postImage} alt="Featured Image" width={400} height={400} className="md:h-50 w-full" />
        )}
      ...
      <div className="flex items-center gap-3">
        {post.authorImage && (
          <Image src={post.authorImage} alt={post.authorName} width={40} height={40} className="size-10 rounded-full object-cover" />
        )}

        <div className="flex flex-col">
          <p>{post.authorName} {post.authorSurname}</p>
